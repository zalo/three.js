...
class ProgressiveLightMap {
	...
	constructor( renderer, res = 1024 ) {
		...
		// Create the Progressive LightMap Textures
		...
		let curRes = this.res / (this.mobile ? 2.0 : 1.0), curLowRes = this.res / (this.mobile ? 4.0 : 2.0);
		
		this.progressiveLightMap1       = new THREE.WebGLRenderTarget( curRes, curRes, { type: format } );
		this.progressiveLightMap2       = new THREE.WebGLRenderTarget( curRes, curRes, { type: format } );
		this.stochasticDepthColorBuffer = new THREE.WebGLRenderTarget( curLowRes, curLowRes, { type: format, magFilter: THREE.NearestFilter, minFilter: THREE.NearestFilter } );
		this.progressiveBounceMap1      = new THREE.WebGLRenderTarget( curLowRes, curLowRes, { type: format } );
		this.progressiveBounceMap2      = new THREE.WebGLRenderTarget( curLowRes, curLowRes, { type: format } );
		this.compositeLightMap1         = new THREE.WebGLRenderTarget( curRes, curRes, { type: format } );
		this.compositeLightMap2         = new THREE.WebGLRenderTarget( curRes, curRes, { type: format } );

		// Inject some spicy new logic into a standard phong material
		this.uvMat = this._createModifiedMaterial(new THREE.MeshPhongMaterial(),
		...
		this.bounceGatherMaterial = this._createBounceGatherMaterial();
	...
	/** Adds these objects to the lightmapping engine.
	...
	/** This function renders each mesh one at a time into the global lightmap.
	...
	update( camera, blendWindow = 100, blurEdges = true, indirectContribution = 1.0, indirectBlendWindow = 5000 ) {
		...
		// Return if there are no objects in the lightmap
		if ( this.lightMapContainers.length === 0 ) { return; }

		// Store the original Render Target
		let oldTarget = this.renderer.getRenderTarget();

		// Configure lightmapping-specific rendering
		this._prepareObjectsForLighting(camera, blurEdges);

		// RenderPass accumulating 8 Directional Lights at once
		this._renderDirectLighting(camera, blendWindow);

		// RenderPass accumulating 1 or 7 Indirect Lighting passes
		if ( indirectContribution > 0 ) {
			for ( let bounceIter = 0; bounceIter < 1 + ( this.mobile ? 0 : 6 ); bounceIter ++ ) {
				this._renderIndirectLighting(indirectBlendWindow);
			}
		}

		// Combine the results of the Direct and Indirect Passes
		this._compositeDirectAndIndirectLighting(camera, indirectContribution, blurEdges);

		// Restore the original settings
		this._resetObjectLightingState();
	...
	_prepareObjectsForLighting(camera, blurEdges) {
		// The blurring plane applies blur to the seams of the lightmap
		this.blurringPlane.visible = blurEdges;

		// Manually set all of the lights to update their shadows
		...
		// This is necessary; I'm not sure exactly why...
		if ( this.firstUpdate ) {
	...
	/** A pass that samples and accumulates a set of lights. */
	_renderDirectLighting(camera, blendWindow) {
		...
		this.activeMap   = this.buffer1Active ? this.progressiveLightMap1 : this.progressiveLightMap2;
		this.inactiveMap = this.buffer1Active ? this.progressiveLightMap2 : this.progressiveLightMap1;
		...
		this.renderer.setRenderTarget( this.activeMap );
		this.uvMat.uniforms.previousShadowMap = { value: this.inactiveMap.texture };
		this.uvMat.uniforms.previousBounceMap = { value: inactiveBounceMap.texture };
		this.blurringPlane.material.uniforms.previousShadowMap = { value: this.inactiveMap.texture };

		this.renderer.render( this.scene, camera );
	}

	/** A pass that stochastically samples global bounce lighting along a single direction. */
	_renderIndirectLighting(indirectBlendWindow) {
		...
		for ( let l = 0; l < this.lightMapContainers.length; l ++ ) {
			...
			this.lightMapContainers[ l ].depthMat.uniforms.time = { value: performance.now() };
		...
		this.bounceGatherMaterial.uniforms.previousShadowMap = { value: this.inactiveMap.texture };
		...
		this.bounceGatherMaterial.uniforms.averagingWindow = { value: indirectBlendWindow };
		...
		// Uniform Hemispherical Surface Distribution to sample all ray directions equally
	...
	/** A pass that combines the direct and indirect passes into a single lightmap */
	_compositeDirectAndIndirectLighting(camera, indirectContribution, blurEdges) {
	...
	/** Restore the objects' original material and add them back to the original scene */
	_resetObjectLightingState() {
		for ( let l = 0; l < this.lightMapContainers.length; l ++ ) {
	...
	/** INTERNAL: Create the material that gathers samples from the stochastic depth. 
	 * @returns {THREE.Material} */
	_createBounceGatherMaterial() {
		...
		return this.bounceGatherMaterial;
	}
	...
	_initializeBlurPlane( res, lightMap = null ) {
		...
		let blurMaterial = this._createModifiedMaterial(new THREE.MeshBasicMaterial(), uniforms,
	...
	_createModifiedMaterial(originalMaterial, uniforms, vertexStart, vertexBody, vertexEnd, fragmentStart, fragmentBody, fragmentEnd, extensions, skip) {
		/** @type {THREE.Material} */
		...
		modifiedMaterial.onBeforeCompile = (shader) => {
			...
			if (!skip) {
				shader.vertexShader = vertexStart + '\n' + shader.vertexShader.slice(0, vertexBodyStart) +
					vertexBody + shader.vertexShader.slice(vertexBodyStart - 1, - 1) + vertexEnd;
				shader.fragmentShader = fragmentStart + '\n' + shader.fragmentShader.slice(0, fragmentBodyStart) +
					fragmentBody + shader.fragmentShader.slice(fragmentBodyStart - 1, - 1) + fragmentEnd;
			}
				Object.assign(shader.uniforms, uniforms);
